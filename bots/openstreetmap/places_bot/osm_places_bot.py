...
# EXTRACT_URL = "https://s3.amazonaws.com/metro-extracts.mapzen.com/berlin.osm.bz2"
# EXTRACT_URL = "https://s3.amazonaws.com/metro-extracts.mapzen.com/odessa.osm.bz2"
# EXTRACT_URL = "map.osm"
EXTRACT_URL = "berdyansk.osm.bz2"
...
class PlaceCollector(object):
    ...
    def ways(self, ways):
        for osmid, tags, refs in ways:
            ...
            if name and len(tags) and refs[0] == refs[-1]:
                osmid = 'osm/w/' + str(osmid)
                self.ways_count += 1
                self.places.append(
                    Place(name=name, id=osmid, tags=tags, refs=refs))
                if self.ways_count % 1000 == 0:
                    print 'ways: ', self.ways_count
    ...
    def show(self):
        ...
        for p in self.places:
            ...
            print p.tags
            print p.id
            db.places.update({'_id': p.id}, {'name': p.name}, upsert=True)
            print '---'
